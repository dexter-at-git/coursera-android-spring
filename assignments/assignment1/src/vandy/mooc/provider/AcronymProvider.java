...
public class AcronymProvider extends ContentProvider {
    ...
    public int bulkInsert(Uri uri,
        ...
        case ACRONYMS:
            ...
            Log.d(TAG,"bulkInsert case ACRONYMS");

            try {
                // TODO+ -- write the code that inserts all the
                // contentValues into the SQLite database.
                for (ContentValues value : contentValues) {
                    final long id = db.insert(AcronymEntry.TABLE_NAME, null, value);
                    if (id != -1)
                        returnCount++;
                }
    ...
    public Cursor query(Uri uri,
        ...
        case ACRONYMS: 
            ...
            // into the method.
            retCursor = mOpenHelper.getReadableDatabase().rawQuery("SELECT * FROM " + AcronymEntry.TABLE_NAME + 
            													   " WHERE " + AcronymEntry.COLUMN_ACRONYM + "=?", selectionArgs);
            break;
        case ACRONYM: 
            ...
            final String rowId =
                ...
                + "'";
            Log.v(TAG,"Cursor query ACRONYM rowId-" + rowId);
