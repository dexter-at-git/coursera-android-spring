...
/**
 * This class implements all the Video-related operations. It implements
 * ConfigurableOps so it can be created/managed by the GenericActivity
 * framework. It implements Callback so it can serve as the target of an
 * asynchronous Retrofit RPC call. It extends GenericAsyncTaskOps so its
...
public class VideoOps implements ConfigurableOps,
	...
	/**
	 * The ListView that contains a list of Videos available from Server.
	 */
		...
		mVideosList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (video != null) {
					...
					final Intent intent = VideoDetailsActivity
							.makeIntent((Video) video);
					intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);

					// Verify that the intent will resolve to an Activity.
					if (intent.resolveActivity(mApplicationContext
							.getPackageManager()) != null)
					{
						...
						Utils.showToast(mActivity.get(),
								"No Activity found to display Video details");
					}
				}
			}
	...
	public void uploadVideo(Long videoId, Uri videoUri) {
		mApplicationContext.startService(UploadVideoService.makeIntent(
				mApplicationContext, videoId, videoUri));
	}

	/**
	 * Retrieve the List of Videos by help of VideoController via a synchronous
	 * two-way method call, which runs in a background thread to avoid blocking
	 * the UI thread.
	 */
	@Override
